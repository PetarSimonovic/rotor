...
class AsteroidGenerator: ObservableObject {
...
//        let sphericalNoise = mapToSphereEquiProjection(vertexList: icosphere.vertices, map: map, radius: radius, mapSize: mapSize)
        ...
        let gravityField: SCNPhysicsField = SCNPhysicsField.drag()
        gravityField.strength = 250
        landscapeNode.physicsBody?.mass = 100.0
        landscapeNode.physicsField = gravityField
        print(gravityField.isActive)
        print(gravityField.strength)
        
        landscapeNode.scale = SCNVector3(x: 20, y: 20, z: 20)
